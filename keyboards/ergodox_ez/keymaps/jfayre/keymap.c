...
//combo keys
const uint16_t PROGMEM caps_lock_both_shift_keys[] = {KC_LSFT, KC_RSFT, COMBO_END};
combo_t key_combos[] = {
	COMBO(caps_lock_both_shift_keys, KC_CAPS_LOCK)
};
...
uint8_t getTapDanceState(tap_dance_state_t *state);

//tap dance hooks for each of our tap dance keycodes
void ctrl_home_finished(tap_dance_state_t *state, void *user_data);
void ctrl_home_reset(tap_dance_state_t *state, void *user_data);
void ctrl_end_finished(tap_dance_state_t *state, void *user_data);
void ctrl_end_reset(tap_dance_state_t *state, void *user_data);
void ad_domain_finished(tap_dance_state_t *state, void *user_data);
void ad_domain_reset(tap_dance_state_t *state, void *user_data);
void handleTDEscapeFlash(tap_dance_state_t *state, void *user_data);
...
[LAYER_QWERTY] = LAYOUT_ergodox_pretty(
	...
	//row 1, right hand
	KC_CAPS_LOCK, KC_6, KC_7, KC_8, KC_9, KC_0, KC_MINS,
	//row 2, left hand
	KC_DEL, KC_Q, KC_W, KC_E, KC_R, KC_T, TG(LAYER_MORE_KEYS),
	//row 2, right hand
	LM(LAYER_MORE_KEYS, MOD_LCTL), KC_Y, KC_U, KC_I, KC_O, KC_P, TD(TD_AD_DOMAIN),
	//row 3, left hand
...
[LAYER_SYMBOLS] = LAYOUT_ergodox_pretty(
	...
	//row 2, left hand
	KC_TRNS, KC_TRNS, BOTH_PARENTHESES, KC_LBRC, KC_RBRC, BOTH_BRACKETS, KC_TRNS,
	//row 2, right hand
	...
	if (record->event.pressed) {
		switch (keycode) {
			...
			case LEFT_BRACE_AT_LINE_END:
				SEND_STRING(SS_TAP(X_END)" {");
				return false;
			case CMD_PREV_DESKTOP:
				SEND_STRING(SS_LCTL(SS_LGUI(SS_TAP(X_LEFT))));
				return false;
			case CMD_NEXT_DESKTOP:
...
tap_dance_action_t tap_dance_actions[] = {
	...
	[TD_AD_DOMAIN] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, ad_domain_finished, ad_domain_reset),
